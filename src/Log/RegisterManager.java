...
public class RegisterManager extends JInternalFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					RegisterManager frame = new RegisterManager();
		...
		surebtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (userMsg.getText() == "" && passMsg.getText() == "" && againMsg.getText() == "") {
					...
					String sql = "insert into Manager(username,password) values(?,?)";// 使用占位符定义插入语句
					try (Connection conn = dbcm.getConnection(); // 获取数据库接
						...
						JOptionPane.showMessageDialog(null,"注册成功，请前往个人中心");
					} catch (SQLException e1) {
